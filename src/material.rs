...
#[derive(Debug)]
pub struct Material<UniformDataType: Default = ()> {
    //pipeline_id: ResourceId<Shader>,
    texture_id: ResourceId<Texture>,
    uniform_data: UniformDataType,
    //_marker_udt: PhantomData<UniformDataType>,
}

impl<UniformDataType: Default> Material<UniformDataType> {
    /// Takes a MaterialBuilder and turns it into a Material
    fn from_builder(builder: MaterialBuilder<UniformDataType>, engine: &mut Engine) -> Self {
        ...
        Self {
            //pipeline_id,
            texture_id,
            uniform_data: UniformDataType::default(),
...
//impl<UniformDataType: Default + ShaderType + WriteInto> Material<UniformDataType> {
...
pub struct MaterialBuilder<UniformDataType: Default = ()> {
...
impl<UniformDataType: Default> MaterialBuilder<UniformDataType> {
    ...
    pub fn add_uniform_data<H: Default + ShaderType + WriteInto>(
        self,
        _data: &UniformData<H>,
    ) -> MaterialBuilder<H> {
        MaterialBuilder {
    ...
    pub fn build(self, engine_handle: &mut Engine) -> Material<UniformDataType> {
        Material::from_builder(self, engine_handle) //::from_builder(self, engine_handle)
    }
}

impl<UniformDataType: Default> Default for MaterialBuilder<UniformDataType> {
    fn default() -> Self {
        Self::new()
    }
}
...
// #[derive(Debug)]
// pub struct Material<VertexType: VertexLayout = Vertex, UniformDataType = ()> {
//     pipeline_id: ResourceId<Shader>,
...
// }
//
// impl<VertexType: VertexLayout> Material<VertexType> {
...
// }
//
// impl<VertexType: VertexLayout, UniformDataType: ShaderType + WriteInto> Material<VertexType, UniformDataType> {
//     /// Attempts to update the uniform data held within in the shader.
//     /// This will fail in the event that the shader has not loaded yet or
//     /// if the shader used to create the material never had any UniformData,
//     pub fn update_uniform_data(
...
//
// /// A builder struct used to create Materials
// pub struct MaterialBuilder<VertexType: VertexLayout, UniformDataType> {
//     // using options to denote a change from the default
//     // in the case of a texture the default is just the White_Pixel
//     texture_change: Option<ResourceId<Texture>>,
//     shader_change: Option<ResourceId<Shader>>,
